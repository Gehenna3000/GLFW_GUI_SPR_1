...
while (!glfwWindowShouldClose(window))
{
    ...
    ImGui::Begin("Edytor Macierzy Splotu");
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            ...
            if (ImGui::InputInt("##cell", &customMatrix[i][j], 0, 0)) {
                changed = true;
            }
    ...
    if (ImGui::Button("Zastosuj Teraz") || changed) {
        MatrixTransform(customMatrix);
    }

    if (ImGui::Button("Reset do Jednostkowej")) {
        for(int i=0; i<3; i++) 
            for(int j=0; j<3; j++) 
                customMatrix[i][j] = (i == j ? 1 : 0);
    }
    ...
    // Czyścimy ekran raz na klatkę
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

    // 5. Ustawienia macierzy i oświetlenia
